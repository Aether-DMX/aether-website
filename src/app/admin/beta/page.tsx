...
export default function AdminBetaPage() {
  ...
  const fetchSignups = useCallback(async () => {
    try {
      const response = await fetch('/api/admin/beta/list');
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0c] pt-8 pb-16">
      {/* Toast */}
      {toast && (
        <div
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <button
            ...
            className="text-[#71717a] hover:text-white transition-colors text-sm flex items-center gap-2"
          >
        ...
        {isLoading ? (
          ...
          <div className="bg-[#111114] border border-[#1f1f24] rounded-xl p-12 text-center">
            ...
            <h3 className="text-white font-semibold mb-2">No pending signups</h3>
            <p className="text-[#71717a] text-sm">All beta signups have been processed.</p>
          </div>
        ) : (
        ...
        {!isLoading && !error && signups.length > 0 && (
          <div className="mt-8 text-center text-[#71717a] text-sm">
            {signups.length} pending signup{signups.length !== 1 ? 's' : ''}
          </div>
        )}
      </div>
    </div>
